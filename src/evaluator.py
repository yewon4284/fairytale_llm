...
PASS_AVG = 4.2      # 평균 합격선
                    # (기존 4.5 -> 4.2로 완화.
                    #  주근거(데이터 비의존): 5점 리커트 척도 구간 산출 관례
                    #  (구간너비 = (최대5-최소1)/5 = 0.8 -> 1.00~1.80/1.81~2.60/2.61~3.40/
                    #  3.41~4.20/4.21~5.00의 5구간, "매우 안전(4.21~5.00)" 만을 합격으로 봄).
                    #  ※ 원 출처(흔히 Best & Kahn으로 인용됨)는 직접 1차 확인은 못 했으니
                    #  논문에 인용 시 원문 대조 필요. 구간 공식 자체는 여러 연구에서
                    #  통용되는 관례임.
                    #  보조근거(참고용, 채택 이유 아님 — 사후 확인): 사람 라벨 421편
                    #  벤치마크에서도 4.2가 4.5 대비 FP 24건 해소/TP 7건 희생으로 순이득이
                    #  가장 큰 구간과 일치함(eval_compare.py --threshold-sweep로 재현 가능).
                    #  단 이 값을 그 스윕으로 "고른" 것이 아니라 위 리커트 관례로 먼저
                    #  정하고 사후에 데이터와 일치함을 확인한 것 — 테스트셋 튜닝(cherry-picking)
                    #  우려를 피하기 위함. 참고: 6개 항목 평균이라 실제 점수는 1/6 단위로만
                    #  나와서 4.2~4.3 사이엔 값이 없으므로 4.2와 4.3은 분류 결과가 동일함.
